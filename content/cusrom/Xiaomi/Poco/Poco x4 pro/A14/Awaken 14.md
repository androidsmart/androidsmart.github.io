---
type   : cusrom
title  : Download Awaken Os Android 14 Poco X4 Pro | Redmi note 11 pro plus
slug   : awakaen-a14-veux
date   : 2024-02-26T09:17:35+07:00
categories: [rom]
tags      : [veux]
keywords  : [poco x4 pro]
author : isaiahscape
---

Download Custom RoM awaken OS Minor 4.3 for Poco X4 Pro / Redmi Note 11 Pro 5G (veux)

## Changelog

- February Security Patch
- Added Moto Dolby Atmos
- Added charging control with tweaks
- Added some overlays, thanks to @lemezohaib's configurations.
- Rashoumon Kernel as default.
- Some minor fixes from 4.3
- Fixed some interface lags
- ROM changelog here

## Bugs

- NFC for CN variants
- Proximity Sensor doesn't work.
- Find and tell me.

## Notes
- Maintaining this ROM crucially.
- Device is certified by default.
- Use latest MIUI 14.x firmware for your region.


## Download Link
[Telegram](https://t.me/wahyu6070files/9?single)
